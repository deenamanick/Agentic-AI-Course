**Architecture Type**: Durable Orchestrator (Workflows / AI)

# AGENTS.md

## Repository
- **Name**: Agentic AI Engineer Course (Jeevi Academy)
- **Architecture Standard**: Durable Orchestrator (See `jeevisoft-architecture` skill)
- **Current Course Labs**: Local-first Python modules using FastAPI, Ollama, LangChain, LangGraph, and Langfuse
- **Production Target**: A standalone Cloudflare Worker that returns `202 Accepted` and delegates long-running agent work to Cloudflare Workflows or Durable Objects
- **Operational State**: Cloudflare D1 for workflow/job status, ownership, and retry metadata
- **Optional Object Storage**: Cloudflare R2 for large course assets, generated artifacts, and agent outputs

## Architecture & Navigation
- **Course Modules (`/module-*`)**: Self-contained local teaching labs. Their FastAPI services are development examples, not the Cloudflare production runtime.
- **Future Frontend (`/frontend`)**: A purely static SPA deployed to Cloudflare Pages. Do not add Pages Functions.
- **Future Backend (`/worker`)**: A standalone Cloudflare Worker responsible for authentication, validation, orchestration, and job-status APIs.
- **Future Workflows (`/workflows`)**: Durable multi-step AI execution, third-party polling, retries, and timeout handling.
- **Future Database Migrations (`/migrations`)**: D1 schema and data changes.
